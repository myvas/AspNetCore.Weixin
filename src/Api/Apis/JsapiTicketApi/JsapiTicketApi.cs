...
namespace Myvas.AspNetCore.Weixin
{
    ...
    {
        public JsapiTicketApi(IOptions<WeixinApiOptions> optionsAccessor, IWeixinAccessToken tokenProvider) : base(optionsAccessor, tokenProvider)
        {
        }
        ...
        /// </exception>
        public async Task<JsapiTicketJson> GetJsapiTicket(string access_token)
        {
            ...
            var result = await GetFromJsonAsync<JsapiTicketJson>(api);
            if (result.Succeeded)
                return result;
            else
