...
namespace Myvas.AspNetCore.Weixin
{
    ...
    {
        public UserApi(IOptions<WeixinApiOptions> optionsAccessor, IWeixinAccessToken tokenProvider) : base(optionsAccessor, tokenProvider)
        {
        }
        ...
        /// </code>
        /// </remarks>
        public async Task< UserInfoJson> Info(string accessToken, string openId, WeixinLanguage lang = WeixinLanguage.zh_CN)
        {
        ...
        /// <returns></returns>
        public async Task<UserGetJson> Get(string accessToken, string nextOpenId)
        {
        ...
        /// <param name="accessToken"></param>
        /// <returns></returns>
        public async Task<List<string>> GetAllOpenIds(string accessToken)
        {
            ...
            do
            {
                UserGetJson followerResult = await Get(accessToken, nextOpenId);
        ...
        /// <param name="accessToken"></param>
        /// <returns></returns>
        public async Task<List<UserInfoJson>> GetAllUserInfo(string accessToken)
        {
            ...
            foreach (string openid in openids)
            {
                try
                {
                    UserInfoJson userInfo = await Info(accessToken, openid);
                    if (userInfo != null)
                    {
