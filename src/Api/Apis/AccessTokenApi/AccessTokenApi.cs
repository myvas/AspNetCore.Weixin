...
namespace Myvas.AspNetCore.Weixin
{
    ...
    /// </remarks>
    public class AccessTokenApi : WeixinApiClient
    {
        private const string GetAccessTokenEndpoint = "https://api.weixin.qq.com/cgi-bin/token";

        public AccessTokenApi(HttpClient client) : base(client)
        {
        }
        ...
        /// </exception>
        public async Task<WeixinAccessTokenJson> GetTokenAsync(string appid, string secret)
        {
            ...
            var result = await Http.GetFromJsonAsync<WeixinAccessTokenJson>(requestUri);
            if (result.Succeeded)
                return result;
            else
                throw new WeixinAccessTokenException(result.ErrorCode.GetValueOrDefault(WeixinResponseStatus.WeixinError), result.ErrorMessage);
        }
    }
}
