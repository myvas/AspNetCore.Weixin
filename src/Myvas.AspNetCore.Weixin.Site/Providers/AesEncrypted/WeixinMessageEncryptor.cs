...
namespace Myvas.AspNetCore.Weixin
{
	public class WeixinMessageEncryptor : IWeixinMessageEncryptor
	{
		...
		private readonly WeixinEncodingOptions _encodingOptions;
		...
		public WeixinMessageEncryptor(IOptions<WeixinAccessTokenOptions> optionsAccessor,
			...
			IOptions<WeixinEncodingOptions> encodingOptionsAccessor,
			ILoggerFactory loggerFactory)
		{
			...
			_siteOptions = siteOptionsAccessor?.Value ?? throw new ArgumentNullException(nameof(siteOptionsAccessor));
			_encodingOptions = encodingOptionsAccessor?.Value ?? throw new ArgumentNullException(nameof(encodingOptionsAccessor));
		}
		...
		public string Decrypt(string msg_signature, string timestamp, string nonce, string data)
		{
			...
			if (!SignatureHelper.ValidateSignature(msg_signature, _siteOptions.WebsiteToken, timestamp, nonce, encryptBody))
			{
		...
		public string Encrypt(string data, string timestamp, string nonce)
		{
			...
			string msg_signature = SignatureHelper.CalculateSignature(_siteOptions.WebsiteToken, timestamp, nonce, raw);
