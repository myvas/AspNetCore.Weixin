...
public class WeixinSubscriberSyncServiceTests
{
    ...
    public WeixinSubscriberSyncServiceTests()
    {
    ...
    [Fact]
    public async Task SubscribeAndPull()
    {
        ...
        var syncService = testServer.Services.GetRequiredService<IWeixinSubscriberSyncService>();
        Assert.NotNull(syncService);
        await syncService.PullSubscribersAsync();

        Debug.WriteLine("Begin to wait for Sync in 1 seconds...");
        await WaitingAsync(TimeSpan.FromSeconds(1), () =>
        {
            Debug.WriteLine(".");
        });
        ...
        var syncSubscriber3 = await db.WeixinSubscribers.FirstOrDefaultAsync(x => x.OpenId == "OPENID1");
        Assert.True(syncSubscriber3.Subscribed);
        var syncSubscriber4 = await db.WeixinSubscribers.FirstOrDefaultAsync(x => x.OpenId == "OPENID2");
        Assert.False(syncSubscriber4.Subscribed);
    }
